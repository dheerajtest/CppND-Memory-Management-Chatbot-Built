...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            //new edge
                        	std::unique_ptr<GraphEdge> e = std::make_unique<GraphEdge>(id);
                        	
                            e->SetChildNode((*childNode).get());
                            e->SetParentNode((*parentNode).get());

                            // all keywords for current node
                            AddAllTokensToElement("KEYWORD", tokens, *e);

                            // store reference in child node and parent node
                            (*childNode)->AddEdgeToParentNode(e.get());
                            (*parentNode)->AddEdgeToChildNode(std::move(e));
                        }
	...
	ChatBot ch = ChatBot("../images/chatbot.png");
	
	ch.SetChatLogicHandle(this);
	
	ch.SetRootNode(rootNode); // add chatbot to graph root node
	
    _chatBot = &ch;
    
    rootNode->MoveChatbotHere(std::move(ch));
